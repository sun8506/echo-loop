...
const libraryRequested = isNativeApp
  ? new URL(window.location.href).searchParams.get('screen') === 'courses'
  : /^\/learn\/courses\/?$/.test(window.location.pathname)
...
export default function LearnerPortal() {
  ...
  if (libraryRequested) return <CourseImport/>
  ...
  return <main className="learner-portal">
    ...
    <nav className="learner-bottom-nav"><button className="active"><BookOpen/>学习</button><button onClick={openLibrary}><Import/>素材</button></nav>
  </main>
}
